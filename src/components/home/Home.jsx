                ...
                <div className="container mx-auto max-w-[1440px] p-5 flex justify-center items-center">
                    <div className="w-full">
                        <form className="relative">
                            <input type="text" value={searchText} onChange={handleSearchChange} className="w-full py-2 pl-10 pr-4 text-gray-700 bg-white border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent font-jakarta" placeholder="Search recipes..." />
                            ...
                            recipes.map((recipe) => (
                                <Link key={recipe.id} to={`/recipe/${recipe.id}`}>
                                    <div className="w-[453px] h-[312px] recipe-card bg-gray-100 rounded-lg shadow-md overflow-hidden transition-transform transform hover:translate-y-[-5px]">
                                        <img className="recipe-image w-full h-48 object-cover transition-transform duration-300 transform hover:scale-105" src={recipe.image} />
